...
- Concurrent refresh: fire eligibility requests continuously while a background refresh swaps the in-memory snapshot, and assert every response reflects either the pre- or post-refresh data cleanly — never a torn read (a request seeing part of the old map and part of the new one) and never a request-visible pause while the swap happens.
