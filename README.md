...
**Branching:** work happens on short-lived feature branches off `main` (e.g. `f1`), merged via PR after review. `main` is protected — no direct pushes. Every PR must show a green `mvn test` run before merge; the PR description records the pass/fail count as evidence.
